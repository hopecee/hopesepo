...
import static org.jboss.seam.security.management.picketlink.JpaIdentityStore.OPTION_ATTRIBUTE_CLASS_NAME;
import static org.jboss.seam.security.management.picketlink.JpaIdentityStore.OPTION_CREDENTIAL_CLASS_NAME;
import static org.jboss.seam.security.management.picketlink.JpaIdentityStore.OPTION_IDENTITY_CLASS_NAME;
import static org.jboss.seam.security.management.picketlink.JpaIdentityStore.OPTION_RELATIONSHIP_CLASS_NAME;
import static org.jboss.seam.security.management.picketlink.JpaIdentityStore.OPTION_ROLE_TYPE_CLASS_NAME;
...
public class JpaIdentityStore implements org.picketlink.idm.spi.store.IdentityStore, Serializable {
     ...
     */
    private Map<String, JpaIdentityStore.MappedAttribute> attributeProperties = new HashMap<String, JpaIdentityStore.MappedAttribute>();
    ...
    public void bootstrap(IdentityStoreConfigurationContext configurationContext)
        ...
        System.out.println("=bootstrap====b ======bgvgbh===========  : ");
    ...
    protected void configureIdentityName() throws IdentityException {
        List<Property<Object>> props = PropertyQueries.createQuery(identityClass)
                .addCriteria(new JpaIdentityStore.PropertyTypeCriteria(PropertyType.NAME))
                .getResultList();
    ...
    protected void configureIdentityType() throws IdentityException {
        List<Property<Object>> props = PropertyQueries.createQuery(identityClass)
                .addCriteria(new JpaIdentityStore.PropertyTypeCriteria(PropertyType.TYPE))
                .getResultList();
    ...
    protected void configureCredentials() throws IdentityException {
        ...
        if (credentialClass != null) {
            List<Property<Object>> props = PropertyQueries.createQuery(credentialClass)
                    .addCriteria(new JpaIdentityStore.PropertyTypeCriteria(PropertyType.VALUE))
                    .getResultList();
            ...
            if (props.size() == 1) {
                ...
                props = PropertyQueries.createQuery(credentialClass)
                        .addCriteria(new JpaIdentityStore.PropertyTypeCriteria(PropertyType.CREDENTIAL))
                        .getResultList();
                if (props.size() == 1) {
                    modelProperties.put(PROPERTY_CREDENTIAL_VALUE, props.get(0));
                } else if (props.size() > 1) {
                    ...
                    if (p != null) {
                        modelProperties.put(PROPERTY_CREDENTIAL_VALUE, p);
                    }
                }
            }
            ...
            props = PropertyQueries.createQuery(identityClass)
                    .addCriteria(new JpaIdentityStore.PropertyTypeCriteria(PropertyType.CREDENTIAL_TYPE))
                    .getResultList();
            ...
            if (props.size() == 1) {
                ...
                if (p != null) {
                    modelProperties.put(PROPERTY_CREDENTIAL_TYPE, p);
                }
            }
        }
    ...
    protected void configureRelationships() throws IdentityException {
        ...
        List<Property<Object>> props = PropertyQueries.createQuery(relationshipClass)
                .addCriteria(new TypedPropertyCriteria(identityClass))
                .addCriteria(new JpaIdentityStore.PropertyTypeCriteria(PropertyType.RELATIONSHIP_FROM))
                .getResultList();
        ...
        props = PropertyQueries.createQuery(relationshipClass)
                .addCriteria(new TypedPropertyCriteria(identityClass))
                .addCriteria(new JpaIdentityStore.PropertyTypeCriteria(PropertyType.RELATIONSHIP_TO))
                .getResultList();
    ...
    protected void configureAttributes() throws IdentityException {
        ...
        List<Property<Object>> props = PropertyQueries.createQuery(identityClass)
                .addCriteria(new JpaIdentityStore.PropertyTypeCriteria(PropertyType.ATTRIBUTE))
                .getResultList();
        ...
        for (Property<Object> p : props) {
            ...
            attributeProperties.put(attribName, new JpaIdentityStore.MappedAttribute(null, p));
        }
        ...
        for (Property<Object> p : props) {
            if (!p.isReadOnly() && p.getJavaClass().isAnnotationPresent(Entity.class)) {
                ...
                for (Property<Object> attributeProperty : pp) {
                    ...
                    attributeProperties.put(attribName, new JpaIdentityStore.MappedAttribute(p, attributeProperty));
                }
            }
        }
    }
    ...
    public IdentityObject createIdentityObject(
        ...
        try {
            ...
            for (String attribName : attributeProperties.keySet()) {
                JpaIdentityStore.MappedAttribute attrib = attributeProperties.get(attribName);
    ...
    public Collection<IdentityObject> findIdentityObject(
        ...
        JpaIdentityStore.EntityToSpiConverter converter = new JpaIdentityStore.EntityToSpiConverter();
    ...
    public boolean validateCredential(IdentityStoreInvocationContext ctx,
        ...
        if (credentialClass != null) {
            ...
            for (Object result : results) {
                ...
                //This is used.
                try {
                    boolean matched = cryptoService.validatePassword(credential.getValue().toString(), val.toString());
                    System.out.println("matched teCredential===d==h================= : " + matched);
                    if (matched) {
                        return true;
                    }
                } catch (NoSuchAlgorithmException ex) {
                    Logger.getLogger(JpaIdentityStore.class.getName()).log(Level.SEVERE, null, ex);
        ...
        try {
            ...
            for (IdentityObjectAttribute attrib : attributes) {
                if (attributeProperties.containsKey(attrib.getName())) {
                    JpaIdentityStore.MappedAttribute mappedAttribute = attributeProperties.get(attrib.getName());
    ...
    /**
     * Returns an attribute value stored elsewhere than the IDENTITY_ATTRIBUTES
     * table
     *
     * @param ctx
     * @param identity
    ...
    private IdentityObjectAttribute getMappedAttribute(IdentityStoreInvocationContext ctx,
            IdentityObject identity, String name) throws IdentityException {
        JpaIdentityStore.MappedAttribute mappedAttribute = attributeProperties.get(name);
    ...
    /**
     * Removes the attributes specified by the attributeNames property. Mapped
     * attributes cannot be removed via this method, instead their values must
     * be overwritten using updateAttributes()
     */
    @Override
