...
function getBusyOverlay(parent,overlay,busy) {
	if((typeof(parent)==='object' || parent=='viewport') && document.getElementsByTagName) {
		...
		if(typeof(parent)==='object') {
			...
			//I added these below.
                     //set the following keys on the html;   top:=0; marginTop:=0; marginLeft:=0;
                    if(busy['top']=== undefined){ 
                        canvas.style.top='50%'; canvas.style.left='50%'; canvas.style.marginTop='-'+(size/2)+'px'; canvas.style.marginLeft='-'+(size/2)+'px';
			}else{
                        canvas.style.top=busy['top']+'%'; canvas.style.left='50%'; canvas.style.marginTop=busy['marginTop']+'px'; canvas.style.marginLeft=busy['marginLeft']+'px';
			 }
                        //end
